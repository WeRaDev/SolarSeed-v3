...
16. **TRL5 Nextcloud LocalAI backends are volume-installed, not image-baked**: AIO community container `nextcloud-aio-local-ai` (`ghcr.io/docjyj/aio-local-ai-vulkan`) can be healthy/ready on `:10078` with models present and still fail chat/STT if `/backends` is empty. Install with `docker exec nextcloud-aio-local-ai /local-ai backends install llama-cpp` and `... install whisper`, then restart. NC uses `integration_openai` -> `http://nextcloud-aio-local-ai:10078`.
17. **LocalAI healthcheck port must match LOCALAI_ADDRESS**: community config sets `LOCALAI_ADDRESS=:10078` but image default healthcheck is `http://localhost:8080/readyz`, which marks the container unhealthy forever. Set `HEALTHCHECK_ENDPOINT=http://localhost:10078/readyz` and recreate container without wiping volumes. See `ops/RUNBOOK.md` "TRL5 Nextcloud LocalAI restore (2026-08-16)".
18. **TRL5 local LLM for Nextcloud Assistant is AIO LocalAI, not University llama.cpp**: on `wera-ss-pt-tv-1` there was no separate Ollama/llama.cpp service for NC Assistant. Do not confuse host `:8080` (AIO mastercontainer) with LocalAI.
19. **TRL5 outbound mail is Proton Bridge + docker-gateway socat, not a container MTA**: Bridge binds `127.0.0.1:1025` (STARTTLS), mailbox `cloud@wera.global`. Containers reach it via `protonmail-smtp-docker-proxy.service` on `172.18.0.1:1025` / `172.19.0.1:1025`. NC must use `mail_smtpsecure=tls` (not `ssl`); `IMailer::send()` returns failed recipients (empty array = success). Odoo 19 needs `mail.alias.domain` plus bounce/catchall aliases equal to the Bridge mailbox (`cloud`) — `bounce@` yields Bridge `invalid return path` / SMTP 554. System partners must not use `odoobot@example.com`. Bridge can be `signed out`/`locked` after restart — `tmux attach` as `protonmail`, `login 0`, 2FA. Verified 2026-08-16 (NC test + Odoo invite). Full procedure: `ops/RUNBOOK.md` "TRL5 outbound email".
