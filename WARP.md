...
19. **TRL5 outbound mail is Proton Bridge + docker-gateway socat, not a container MTA**: Bridge binds `127.0.0.1:1025` (STARTTLS). Containers reach it via `protonmail-smtp-docker-proxy.service` on `172.18.0.1:1025` / `172.19.0.1:1025`. NC must use `mail_smtpsecure=tls` (not `ssl`). NC `IMailer::send()` returns failed recipients (empty array = success). Bridge account can be `signed out` after restart — operator must `tmux attach` as `protonmail` and `login 0` with Proton password + 2FA, then refresh Bridge SMTP password into NC/Odoo. Full procedure: `ops/RUNBOOK.md` "TRL5 outbound email".
